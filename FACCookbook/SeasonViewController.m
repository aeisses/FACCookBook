...
- (id)recipes {
    ...
    if (_searchString && ![_searchString isEqualToString:@""]) {
        fetchRequest.predicate = [NSPredicate predicateWithFormat:@"(season == %@ or season == %@) and ANY searchItems.item CONTAINS[cd] %@",[SeasonColors convertToString:[Utils getCurrentSeason]],[SeasonColors convertToString:[Utils getCurrentSeason]].lowercaseString,_searchString];
    } else {
